

<?php $__env->startSection('title', 'Mi Tarjeta QR - La Zarza Contigo'); ?>

<?php $__env->startPush('styles'); ?>
<style>
    body {
        background: transparent !important;
    }
    main {
        padding: 0 !important;
        max-width: 100% !important;
        margin: 0 !important;
    }
    .hero-overlay {
        background: linear-gradient(135deg,
            rgba(113, 57, 141, 0.75) 0%,
            rgba(181, 26, 138, 0.65) 50%,
            rgba(0, 0, 0, 0.55) 100%);
    }
    .qr-card {
        background: rgba(255, 255, 255, 0.12);
        backdrop-filter: blur(16px);
        -webkit-backdrop-filter: blur(16px);
        border: 1px solid rgba(255, 255, 255, 0.25);
    }
    .qr-code-container {
        background: #ffffff;
        border-radius: 16px;
        padding: 20px;
        display: inline-block;
        box-shadow: 0 8px 32px rgba(0,0,0,0.2);
    }
    .qr-img {
        width: 220px;
        height: 220px;
        display: block;
    }
    .code-badge {
        font-family: 'Courier New', Courier, monospace;
        font-size: 0.875rem;
        letter-spacing: 0.15em;
    }
    .info-row {
        border-bottom: 1px solid rgba(255,255,255,0.15);
    }
    .info-row:last-child {
        border-bottom: none;
    }
</style>
<?php $__env->stopPush(); ?>

<?php $__env->startSection('content'); ?>
<div class="min-h-screen bg-cover bg-center bg-fixed bg-no-repeat" style="background-image: url('/PROPORCIONAL FHD.jpg');">
<div class="min-h-screen hero-overlay py-8 px-4 sm:px-6 lg:px-8">
<div class="max-w-xl mx-auto">

    
    <div class="qr-card rounded-2xl text-white shadow-xl overflow-hidden">

        
        <div class="px-6 pt-6 pb-4 flex items-center justify-between">
            <div>
                <p class="text-sm text-white/70 uppercase tracking-wider font-semibold font-mercurius">La Zarza Contigo</p>
                <h1 class="text-2xl font-bold mt-1">
                    <?php echo e($usuario->nombres); ?> <?php echo e($usuario->apellido_paterno); ?>

                </h1>
                <p class="text-white/60 text-sm mt-0.5">
                    Miembro desde <?php echo e($usuario->created_at->format('M Y')); ?>

                </p>
            </div>
            <div class="w-14 h-14 rounded-full bg-white/20 flex items-center justify-center">
                <i class="fas fa-user text-2xl text-white/80"></i>
            </div>
        </div>

        
        <div class="flex flex-col items-center py-8 bg-black/10">
            <div class="qr-code-container">
                <img
                    id="qr-image"
                    src="<?php echo e(route('qr.usuario', $usuario->qr_codigo)); ?>"
                    alt="Código QR personal"
                    class="qr-img"
                    onerror="this.src='https://api.qrserver.com/v1/create-qr-code/?size=220x220&data=<?php echo e(urlencode('ZRZ|' . $usuario->qr_codigo)); ?>&bgcolor=ffffff&color=71398d&margin=10'"
                />
            </div>
            <p class="mt-4 text-white/60 text-sm text-center px-6">
                Muestra este código en la sucursal para que te identifiquen
            </p>
            <div class="mt-3 bg-white/20 rounded-full px-5 py-2">
                <span class="code-badge text-white font-bold"><?php echo e($usuario->qr_codigo); ?></span>
            </div>
        </div>

        
        <div class="px-6 py-4 space-y-3">
            <div class="info-row flex justify-between items-center pb-3">
                <span class="text-white/60 text-sm"><i class="fas fa-envelope mr-2"></i>Correo</span>
                <span class="text-sm font-medium text-white"><?php echo e($usuario->email); ?></span>
            </div>
            <div class="info-row flex justify-between items-center pb-3">
                <span class="text-white/60 text-sm"><i class="fas fa-phone mr-2"></i>Teléfono</span>
                <span class="text-sm font-medium text-white"><?php echo e($usuario->telefono); ?></span>
            </div>
            <div class="flex justify-between items-center">
                <span class="text-white/60 text-sm"><i class="fas fa-tags mr-2"></i>Promociones activas</span>
                <span class="text-sm font-bold text-white bg-white/20 rounded-full px-3 py-0.5">
                    <?php echo e($cuponesDisponibles); ?>

                </span>
            </div>
        </div>

        
        <div class="px-6 pt-2 pb-5">
            <button
                onclick="window.print()"
                class="w-full bg-white/20 hover:bg-white/30 text-white font-semibold rounded-xl py-3 transition text-sm mt-2"
            >
                <i class="fas fa-print mr-2"></i> Imprimir mi tarjeta
            </button>
        </div>
    </div>

    
    <div class="mt-6 rounded-xl p-5 space-y-3" style="background: rgba(255,255,255,0.10); backdrop-filter: blur(10px); border: 1px solid rgba(255,255,255,0.2);">
        <h3 class="font-semibold text-white flex items-center gap-2">
            <i class="fas fa-info-circle"></i> ¿Cómo usar mi QR?
        </h3>
        <ol class="list-decimal list-inside text-pink-100 text-sm space-y-1.5 leading-relaxed">
            <li>Muestra la pantalla de tu celular al personal de la sucursal.</li>
            <li>Ellos escanearán el código para identificar tu cuenta.</li>
            <li>Se aplicarán las promociones vigentes a tu compra.</li>
            <li>También puedes imprimir esta tarjeta para llevarla contigo.</li>
        </ol>
    </div>

    
    <div class="mt-4 grid grid-cols-2 gap-3">
        <a href="<?php echo e(route('tickets.create')); ?>"
           class="flex items-center justify-center gap-2 bg-white/20 hover:bg-white/30 text-white font-semibold rounded-xl py-3 text-sm shadow hover:shadow-md transition border border-white/20">
            <i class="fas fa-receipt"></i> Registrar ticket
        </a>
        <a href="<?php echo e(route('coupons.index')); ?>"
           class="flex items-center justify-center gap-2 bg-white/20 hover:bg-white/30 text-white font-semibold rounded-xl py-3 text-sm shadow hover:shadow-md transition border border-white/20">
            <i class="fas fa-tags"></i> Promociones
        </a>
    </div>

</div>
</div>
</div>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\appwebzarza\resources\views/client/mi-tarjeta.blade.php ENDPATH**/ ?>