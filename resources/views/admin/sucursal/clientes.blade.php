...
<div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                ...
                <p class="text-gray-500 mt-1">
                    {{ $admin->sucursal ? $admin->sucursal->nombre : '' }} · Total: {{ $totalRegistrados }} clientes
                </p>
            </div>
            <a href="{{ route('admin.clientes.registrar') }}" class="inline-flex items-center px-5 py-2.5 rounded-lg text-white font-medium transition-all hover:shadow-lg" style="background: linear-gradient(135deg, #1a1a2e 0%, #0f3460 100%);">
    ...
    <div class="bg-white rounded-lg shadow p-4">
        <form method="GET" action="{{ route('admin.mi-sucursal.clientes') }}" class="flex gap-3">
            ...
            @if(request('buscar'))
                <a href="{{ route('admin.mi-sucursal.clientes') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg text-sm hover:bg-gray-300">
                    Limpiar
                </a>
            @endif
        </form>
    ...
    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
        <div class="px-4 pt-4 pb-3 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center gap-3">
            <span class="text-sm text-gray-500">{{ $clientes->total() }} clientes</span>
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4">
                            ...
                            <div class="text-xs text-gray-500">RFC: {{ $cliente->rfc ?? '-' }}</div>
                        </td>
